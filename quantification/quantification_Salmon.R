...
arguments <- parse_args(OptionParser(usage = "%prog",
                                     ...
                                     option_list=list(
                                       ...
                                       make_option(c("-g", "--gene_map"), default = NULL, help = "Call flag to enable quantification and gene level. If enabled, must supply a file containing a mapping of transcripts to genes. Salmon will then output both quant.sf and quant.genes.sf files, where the latter contains aggregated gene-level abundance estimates. The transcript to gene mapping should be provided as a GTF file. In GTF/GFF format, the 'transcript_id' is assumed to contain the transcript identifier and the 'gene_id' is assumed to contain the corresponding gene identifier.")))),
...
# arguments$args[3] <- "/data/RNAseq_PD/tissue_polyA_samples/salmon_quant"
